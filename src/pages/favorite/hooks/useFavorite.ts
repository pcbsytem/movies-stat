...
import {
  HandleFavoriteRemoveProps,
  handleExistInFavoriteProps
} from '../Favorite.types';
...
export const useFavorite = () => {
  ...
  useEffect(() => {
    const handleFavoriteLoad = () => {
      const favoriteStoredValue = localStorage.getItem(
        FAVORITE_LOCAL_STORE_KEY
      );
      dispatch(
        addFavoriteMovies(
          favoriteStoredValue !== null
            ? [...JSON.parse(favoriteStoredValue)]
            : []
        )
      );
    };
  ...
  const handleFavoriteRemove = useCallback(
    ({ id }: HandleFavoriteRemoveProps): void => {
  ...
  const handleFavoriteAdd = useCallback(
    (movie: MovieProps) => {
      ...
      const alreadyAdded = favorite.findIndex(
  ...
  return useMemo(
    ...
    [favorite, handleExistInFavorite, handleFavoriteAdd, handleFavoriteRemove]
  );
};
